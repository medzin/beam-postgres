...
class TestWriteToPostgres:
    ...
    def test_if_write_errors_are_returned(self, mock_connect: mock.Mock):
        ...
        with TestPipeline() as p:
            ...
            assert_that(errors, equal_to([(("test_1",), IntegrityError())], failed_rows_equal_fn))
    ...
    def test_if_works_with_non_global_windows(self, mock_connect: mock.Mock):
        ...
        with TestPipeline() as p:
            errors = (
                ...
                >> WriteToPostgres("host=localhost dbname=examples user=postgres password=postgres", "dummy")
            )
            assert_that(errors, equal_to([(("test_1",), IntegrityError())], failed_rows_equal_fn))


def failed_rows_equal_fn(expected: Tuple[Tuple[str], Error], actual: Tuple[Tuple[str], Error]):
